...
HAPError HandleLightBulbOnRead(
        ...
        bool* value,
        void* _Nullable context HAP_UNUSED) {
    accessoryConfiguration.state.lightBulbOn = ReadDeviceStatus();
...
HAPError HandleLightBulbOnWrite(
    ...
    if (CheckAuthorization(request->authorizationData.bytes, 
                            RIGHT_lightBulbOnCharacteristic_WRITE) == 0) {
    ...
    {
        printf("[Error:] Not authorized write access.\n");
    }
