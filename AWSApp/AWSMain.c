...
void add_ACL_records(typeAuthorizationData ad, typeRight * rightArray, int numberofRights) {
	...
	for (i = 0; i < numberofRights; i++){
		ret = AppendACL(ad, rightArray[i]);
		if ( 0 != ret){
			printf("AppendACL error, %d...\n", ret);
			exit(-1);
		}
	}
}
...
void remove_ACL_records(typeAuthorizationData ad,  typeRight * rightArray, int numberofRights) {
	...
	for (i = 0; i < numberofRights; i++){	
		ret = DeleteACL(ad, rightArray[i]);
		if ( 0 != ret){
			printf("DeleteACL error,%d...\n", ret);
			exit(-1);
		}
	}
}
...
void get_ad_rightArray(typeAuthorizationData ad, typebase64AuthorizationData base64ad, typeRight * rightArray, char * payload, int numberofRights){
	...
	strncpy(base64ad, payload + strlen("*\nad: "), base64AuthorizationDataLength );
	...
	rightSplit = strtok(payload + strlen("*\nad: ") + base64AuthorizationDataLength + strlen("\nrights: "), " ,{}");
...
void iot_subscribe_callback_handler(AWS_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
    ...
    switch(cmd) {
        ...
        case 'H':
            ...
            gChoosedChannel = 1;
            HomeKitThreadSuccess = pthread_create(&gHomeKitChannel, NULL, (void *)EnableChannel, &gChoosedChannel); 
        ...
        case 'h':
			...
			DisableChannel(CHANNEL_HOMEKIT);
		...
		case 'A':
			...
			sscanf(((char*) params->payload) + strlen("*\nad: ") + base64AuthorizationDataLength + strlen("\nrights: "), "%d", &numberofRights);
			if(rightArray != NULL){
				free(rightArray);
				rightArray = NULL;
			}
